...
export const postApis = {
  ...
  postProject: async (selectedData) =>
    await instance.post(`/api/post`, selectedData),
  editProject: async (data) =>
    await instance.put(`/api/post/${data.postId}`, data.data),
  deletePost: async (id) => await instance.delete(`/api/post/${id}`),
  getBookmarkRank: async () => await instance.get("/api/bookMark/rank"),
  getMyBookmarkPost: async () =>
    await instance.get(`/api/user/mypage/bookmark`),
  getMyParticipatePost: async () =>
    await instance.get(`/api/user/participation`),
  ...
  postDeadlineProject: async (postId) =>
    await instance.post(`api/post/${postId}/deadline`),
